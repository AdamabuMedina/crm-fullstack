...
export default class MoneyTransferHistory {
  constructor(account, count, pagination = false) {
    ...
    setChildren(this.header, [headerFrom, headerTo, headerAmount, headerDate])
    mount(this.table, this.header)
    setChildren(this.element, [title, this.table])
  ...
  add(transaction) {
    ...
    if (transaction) {
      ...
      const skeleton = el(".money-transfer-history__row-text-skeleton")

      mount(colFrom, skeleton)
      mount(colTo, skeleton)
      mount(colAmount, skeleton)
      mount(colDate, skeleton)
    }

    setChildren(row, [colFrom, colTo, colAmount, colDate])
    mount(this.table, row)
  }
}
